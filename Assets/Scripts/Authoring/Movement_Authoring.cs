...
public class Movement_Authoring : MonoBehaviour, IConvertGameObjectToEntity
{
    ...
    public void Convert(Entity entity, EntityManager dstManager, GameObjectConversionSystem conversionSystem)
    {
        if ( direction == MovementDirection.Up )
            dstManager.AddComponentData( entity, new LbUpDirection() );
        if (direction == MovementDirection.Down)
            dstManager.AddComponentData( entity, new LbDownDirection() );
        if (direction == MovementDirection.Right)
            dstManager.AddComponentData( entity, new LbRightDirection() );
        if (direction == MovementDirection.Left)
            dstManager.AddComponentData( entity, new LbLeftDirection() );
